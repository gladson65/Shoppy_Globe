...
function Grocery() {
    ...
    useEffect(()=> {
        if (data) {
            setGroceryProduct(data.products);
        }
    }, [data])
                    ...
                    if(item.category == "groceries") {
                        return (
                            <div key={item.id} className="w-full border-4 border-black bg-zinc-100 hover:bg-white rounded-lg flex flex-col items-center">
                                ...
                                <button onClick={()=> addToCart(item)} className="bg-yellow-300 rounded-lg px-2 py-2 w-10/12 md:w-3/5 hover:bg-yellow-400 hover:font-bold transition-all duration-150 ease-linear">
                                    Add To Cart
                                </button>
                                </div>
                            </div>
